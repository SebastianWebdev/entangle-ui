...
<LightThemeOverview client:load />

### Quick start

```ts
// app/light-theme.css.ts
...
`createLightTheme()` must be called from a `.css.ts` file — it generates a Vanilla Extract theme class at build time. The returned class flips every `--etui-*` custom property under the wrapped subtree to the light preset.
...
<LightThemeBasicComponents client:load />

### Property panels

<LightThemePanel client:load />

### Alerts

<LightThemeAlerts client:load />
