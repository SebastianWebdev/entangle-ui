...
The timestamp, level icon, and source tag render as fixed-width columns, so the message text starts at the same x on every row regardless of source length (and on rows that omit a timestamp or source). The two reserved widths default to `88px` (timestamp) and `80px` (source) and are tunable per instance by overriding the CSS custom properties on the root:
