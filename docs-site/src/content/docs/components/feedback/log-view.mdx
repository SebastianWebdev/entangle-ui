...
<ComponentPreview title="Restyling slots with slotProps">
  <LogViewSlotProps client:only="react" />
</ComponentPreview>
...
<ComponentPreview title="Re-skinned via token overrides">
  <LogViewStyled client:only="react" />
</ComponentPreview>
...
<ComponentPreview title="Localized labels (Polish)">
  <LogViewLocalized client:only="react" />
</ComponentPreview>
...
<PropsTable
  props={[
    ...
    {
      name: 'slotProps',
      type: 'LogViewSlotProps',
      description:
        'Per-slot props for the default composition (toolbar, search, levelFilter, copy, clear, body, footer). className/style merge with each slot’s styles; ignored when you pass your own children. Full breakdown in the slotProps reference below.',
    },
    {
      name: 'labels',
      type: 'Partial<LogViewLabels>',
      description:
        'Override built-in UI strings for i18n (search placeholder/label, level-filter label, clear/copy/copy-line labels, jump-to-bottom, new-line counter, empty state, region label). Omitted keys keep their English default. See Internationalization.',
    },
    {
      name: 'onClear',
      type: '() => void',
