...
<ComponentPreview title="Card">
  <SkeletonLayoutCard client:only="react" />
</ComponentPreview>

<ComponentPreview title="List">
  <SkeletonLayoutList client:only="react" />
</ComponentPreview>

<ComponentPreview title="Table">
  <SkeletonLayoutTable client:only="react" />
</ComponentPreview>

<ComponentPreview title="Grid">
  <SkeletonLayoutGrid client:only="react" />
</ComponentPreview>

<ComponentPreview title="Chat">
  <SkeletonLayoutChat client:only="react" />
</ComponentPreview>
...
<PropsTable
  props={[
    {
      name: 'variant',
      type: "'card' | 'list' | 'table' | 'grid' | 'chat'",
      description: 'Which pre-built arrangement to render. Required.',
    },
    {
      name: 'count',
      type: 'number',
      description:
        'Number of repeating units. Defaults per variant: `list` 5 rows, `table` 5 data rows, `grid` 12 cells, `chat` 4 bubbles. Ignored for `card`.',
    },
    {
      name: 'columns',
      type: 'number',
      default: '4',
      description: 'Number of columns. Only meaningful for `grid` and `table`.',
    },
    {
      name: 'animation',
      type: "'pulse' | 'wave' | 'none'",
      description:
        'Animation applied to every nested skeleton. Defaults to `pulse` for `card` / `list` / `table` / `chat`, and `none` for `grid`.',
    },
    {
      name: 'width',
      type: 'number | string',
      default: "'100%'",
      description:
        'Width of the layout container. Number → px, string → CSS value. Defaults to filling the parent so the layout stays stable while content loads.',
    },
  ]}
/>
