---
title: FloatingPanel
description: Draggable, resizable floating panel with collapse/expand, z-index stacking, and controlled/uncontrolled modes.
---

import PropsTable from '../../../../components/PropsTable.astro';
import ComponentPreview from '../../../../components/ComponentPreview.astro';
import FloatingPanelDemo, {
  FloatingPanelManager,
  FloatingPanelDragResize,
  FloatingPanelControlled,
  FloatingPanelUncontrolled,
  FloatingPanelCollapseClose,
  FloatingPanelScrollable,
} from '../../../../components/demos/shell/FloatingPanelDemo';

Draggable and resizable floating panel for editor tool windows, property inspectors, and other detachable UI. Supports drag-to-move via the header bar, corner resize, collapse/expand toggle, and optional close button. Works in both controlled and uncontrolled modes for position, size, and collapsed state. Use with `FloatingManager` for automatic z-index stacking of multiple panels.

<ComponentPreview title="Live Preview">
  <FloatingPanelDemo client:only="react" />
</ComponentPreview>

## Import

```tsx
import { FloatingPanel, FloatingManager } from 'entangle-ui';
```

## Usage

```tsx
<FloatingPanel title="Properties" defaultPosition={{ x: 100, y: 100 }}>
  <p>Panel content goes here.</p>
</FloatingPanel>
```

## FloatingManager

When using multiple floating panels, wrap them in a `FloatingManager` to get automatic z-index stacking. Clicking a panel brings it to the front.

<ComponentPreview title="FloatingManager">
  <FloatingPanelManager client:only="react" />
</ComponentPreview>

```tsx
<FloatingManager baseZIndex={100}>
  <FloatingPanel title="Inspector" defaultPosition={{ x: 50, y: 50 }}>
    <p>Inspector content</p>
  </FloatingPanel>
  <FloatingPanel title="Outliner" defaultPosition={{ x: 400, y: 50 }}>
    <p>Outliner content</p>
  </FloatingPanel>
  <FloatingPanel title="Console" defaultPosition={{ x: 50, y: 400 }}>
    <p>Console content</p>
  </FloatingPanel>
</FloatingManager>
```

## Drag and Resize

Panels are dragged by their header bar. Position is clamped to keep at least 50px visible horizontally and 30px vertically within the viewport.

Resize is handled via the bottom-right corner handle. The resize handle is only visible when `resizable` is `true` (the default) and the panel is not collapsed.

<ComponentPreview title="Drag and Resize">
  <FloatingPanelDragResize client:only="react" />
</ComponentPreview>

```tsx
<FloatingPanel
  title="Resizable Panel"
  defaultSize={{ width: 350, height: 250 }}
  minWidth={200}
  minHeight={150}
  maxWidth={600}
  maxHeight={500}
  resizable
>
  <p>Resize me from the bottom-right corner.</p>
</FloatingPanel>
```

## Controlled Mode

For full control over position, size, and collapsed state, use the controlled props.

<ComponentPreview title="Controlled Mode">
  <FloatingPanelControlled client:only="react" />
</ComponentPreview>

```tsx
const [position, setPosition] = useState({ x: 100, y: 100 });
const [size, setSize] = useState({ width: 300, height: 200 });
const [collapsed, setCollapsed] = useState(false);

<FloatingPanel
  title="Controlled Panel"
  position={position}
  onPositionChange={setPosition}
  size={size}
  onSizeChange={setSize}
  collapsed={collapsed}
  onCollapsedChange={setCollapsed}
>
  <p>Fully controlled.</p>
</FloatingPanel>;
```

## Uncontrolled Mode

For simpler use cases, use the `default*` props and let the component manage its own state.

<ComponentPreview title="Uncontrolled Mode">
  <FloatingPanelUncontrolled client:only="react" />
</ComponentPreview>

```tsx
<FloatingPanel
  title="Uncontrolled Panel"
  defaultPosition={{ x: 200, y: 150 }}
  defaultSize={{ width: 280, height: 200 }}
  defaultCollapsed={false}
>
  <p>Manages its own state.</p>
</FloatingPanel>
```

## Collapse and Close

The header always shows a collapse/expand toggle. The close button only appears when both `closable` is `true` (the default) and an `onClose` handler is provided.

<ComponentPreview title="Collapse and Close">
  <FloatingPanelCollapseClose client:only="react" />
</ComponentPreview>

```tsx
const [visible, setVisible] = useState(true);

{
  visible && (
    <FloatingPanel
      title="Closable Panel"
      closable
      onClose={() => setVisible(false)}
    >
      <p>Click the X to close.</p>
    </FloatingPanel>
  );
}
```

## Scrollable Content

Panel content is automatically wrapped in a `ScrollArea` that scrolls in both directions when content overflows.

<ComponentPreview title="Scrollable Content">
  <FloatingPanelScrollable client:only="react" />
</ComponentPreview>

## Props

### FloatingPanel

<PropsTable
  props={[
    {
      name: 'title',
      type: 'string',
      required: true,
      description: 'Panel title displayed in the header bar.',
    },
    {
      name: 'position',
      type: '{ x: number; y: number }',
      description: 'Controlled position in pixels.',
    },
    {
      name: 'defaultPosition',
      type: '{ x: number; y: number }',
      default: '{ x: 100, y: 100 }',
      description: 'Initial position for uncontrolled mode.',
    },
    {
      name: 'onPositionChange',
      type: '(position: { x: number; y: number }) => void',
      description: 'Called when position changes during drag.',
    },
    {
      name: 'size',
      type: '{ width: number; height: number }',
      description: 'Controlled size in pixels.',
    },
    {
      name: 'defaultSize',
      type: '{ width: number; height: number }',
      default: '{ width: 280, height: 200 }',
      description: 'Initial size for uncontrolled mode.',
    },
    {
      name: 'onSizeChange',
      type: '(size: { width: number; height: number }) => void',
      description: 'Called when size changes during resize.',
    },
    {
      name: 'minWidth',
      type: 'number',
      default: '150',
      description: 'Minimum panel width in pixels.',
    },
    {
      name: 'minHeight',
      type: 'number',
      default: '100',
      description: 'Minimum panel height in pixels.',
    },
    {
      name: 'maxWidth',
      type: 'number',
      default: 'Infinity',
      description: 'Maximum panel width in pixels.',
    },
    {
      name: 'maxHeight',
      type: 'number',
      default: 'Infinity',
      description: 'Maximum panel height in pixels.',
    },
    {
      name: 'collapsed',
      type: 'boolean',
      description: 'Controlled collapsed state.',
    },
    {
      name: 'defaultCollapsed',
      type: 'boolean',
      default: 'false',
      description: 'Initial collapsed state for uncontrolled mode.',
    },
    {
      name: 'onCollapsedChange',
      type: '(collapsed: boolean) => void',
      description: 'Called when collapsed state changes.',
    },
    {
      name: 'onClose',
      type: '() => void',
      description: 'Called when the close button is clicked.',
    },
    {
      name: 'closable',
      type: 'boolean',
      default: 'true',
      description:
        'Whether to show the close button (requires onClose handler to actually render).',
    },
    {
      name: 'resizable',
      type: 'boolean',
      default: 'true',
      description:
        'Whether the panel can be resized via the bottom-right corner handle.',
    },
    {
      name: 'panelId',
      type: 'string',
      description:
        'Unique panel ID for FloatingManager z-index tracking. Auto-generated if not provided.',
    },
    { name: 'children', type: 'ReactNode', description: 'Panel body content.' },
    {
      name: 'className',
      type: 'string',
      description: 'Additional CSS class names.',
    },
    { name: 'style', type: 'CSSProperties', description: 'Inline styles.' },
    {
      name: 'testId',
      type: 'string',
      description: 'Test identifier for automated testing.',
    },
    {
      name: 'ref',
      type: 'Ref<HTMLDivElement>',
      description: 'Ref to the panel root element.',
    },
  ]}
/>

### FloatingManager

<PropsTable
  props={[
    {
      name: 'baseZIndex',
      type: 'number',
      default: '100',
      description:
        'Base z-index value. Each panel stacks one level above the previous.',
    },
    {
      name: 'children',
      type: 'ReactNode',
      description: 'FloatingPanel components to manage.',
    },
  ]}
/>

## Accessibility

- The panel root has `role="dialog"` with `aria-modal="false"` (non-modal dialog)
- `aria-label` is set to the panel title for screen reader identification
- The collapse toggle button has `aria-label` that updates to "Expand panel" or "Collapse panel"
- The close button has `aria-label="Close panel"`
- Both the collapse and close buttons are keyboard-focusable with `tabIndex={0}`
