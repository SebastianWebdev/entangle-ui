...
<PropsTable props={[
  { name: 'orientation', type: '{ yaw: number; pitch: number; roll?: number }', required: true, description: 'Current viewport orientation in degrees. The gizmo always reflects this value (fully controlled).' },
  { name: 'upAxis', type: "'y-up' | 'z-up'", default: "'y-up'", description: 'Which axis points up. Y-up for Three.js/Babylon/Maya, Z-up for Blender/Unreal/CAD.' },
  { name: 'axisColorPreset', type: "'blender' | 'unreal' | 'custom'", default: "'blender'", description: 'Axis color convention. Use "custom" with axisConfig for custom colors.' },
  { name: 'axisConfig', type: '[GizmoAxisConfig, GizmoAxisConfig, GizmoAxisConfig]', description: 'Per-axis configuration [X, Y, Z] with custom label, color, and visibility. Used when axisColorPreset is "custom".' },
  { name: 'showLabels', type: 'boolean', default: 'true', description: 'Whether to show axis labels (X, Y, Z) at arm tips.' },
  { name: 'showNegativeAxes', type: 'boolean', default: 'true', description: 'Whether to show negative axis arms (dimmed, shorter).' },
  { name: 'showOrbitRing', type: 'boolean', default: 'true', description: 'Whether to show a thin orbit ring around the gizmo.' },
  { name: 'showOriginHandle', type: 'boolean', default: 'true', description: 'Whether to show a clickable origin handle at the center.' },
  { name: 'background', type: "'transparent' | 'subtle' | 'solid'", default: "'subtle'", description: 'Background style of the gizmo container.' },
  { name: 'interactionMode', type: "'full' | 'snap-only' | 'orbit-only' | 'display-only'", default: "'full'", description: 'Which interactions are enabled.' },
  { name: 'orbitSpeed', type: 'number', default: '1', description: 'Orbit speed multiplier for drag interactions.' },
  { name: 'constrainPitch', type: 'boolean', default: 'true', description: 'Whether to constrain pitch rotation to [-90, 90] degrees.' },
  { name: 'onOrbit', type: '(delta: { deltaYaw: number; deltaPitch: number }) => void', description: 'Called continuously while the user drags to orbit.' },
  { name: 'onOrbitEnd', type: '(finalOrientation: GizmoOrientation) => void', description: 'Called when the user finishes an orbit drag.' },
  { name: 'onSnapToView', type: "(view: 'front' | 'back' | 'left' | 'right' | 'top' | 'bottom') => void", description: 'Called when the user clicks an axis to snap to a preset view.' },
  { name: 'onAxisClick', type: "(axis: 'x' | 'y' | 'z', positive: boolean) => void", description: 'Called when the user clicks a specific axis arm.' },
  { name: 'onOriginClick', type: '() => void', description: 'Called when the user clicks the origin handle.' },
  { name: 'diameter', type: 'number', default: '120', description: 'Gizmo diameter in pixels.' },
  { name: 'size', type: "'sm' | 'md' | 'lg'", default: "'md'", description: 'Component size, affecting label font size.' },
  { name: 'disabled', type: 'boolean', default: 'false', description: 'Whether the gizmo is disabled.' },
  { name: 'className', type: 'string', description: 'Additional CSS class names on the wrapper element.' },
  { name: 'style', type: 'CSSProperties', description: 'Inline styles on the wrapper element.' },
  { name: 'testId', type: 'string', description: 'Test identifier for automated testing.' },
  { name: 'id', type: 'string', description: 'HTML id attribute on the wrapper element.' },
]} />
