...
<AssetBrowser
  ...
  renderItemContextMenu={selected => (
    <>
      ...
      <Menu.Item onClick={() => rename(selected[0])} disabled={selected.length !== 1}>
        Rename
      </Menu.Item>
      <Menu.Separator />
      <Menu.Item onClick={() => remove(selected)}>Delete</Menu.Item>
    </>
  )}
...
<AssetBrowser
  ...
  labels={{
    ...
    sortFields: { name: 'Nazwa', type: 'Typ', size: 'Rozmiar', modified: 'Zmodyfikowano' },
