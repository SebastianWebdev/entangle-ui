---
title: NodeGraph
description: Data-driven node editor surface — ports, Bézier edges, multi-select, drag, marquee, snap-to-grid, connection validation, keyboard nav, groups, a minimap slot, a toolbar slot, a spawn palette, and an imperative camera handle. Composes the Viewport primitive.
---
...
import NodeGraphBasicsDemo from '../../../../components/demos/editor/NodeGraphBasicsDemo';
import NodeGraphAnatomyDemo from '../../../../components/demos/editor/NodeGraphAnatomyDemo';
import NodeGraphEdgeStyleDemo from '../../../../components/demos/editor/NodeGraphEdgeStyleDemo';
import NodeGraphCameraDemo from '../../../../components/demos/editor/NodeGraphCameraDemo';
import NodeGraphSpawnDemo from '../../../../components/demos/editor/NodeGraphSpawnDemo';
import NodeGraphBenchmarkDemo from '../../../../components/demos/editor/NodeGraphBenchmarkDemo';
...
<PreviewToggle
  ...
  description="A UE5 Blueprint-flavoured kitchen sink. Drag pins to connect (type-checked), drag nodes (edges follow live), marquee-select empty background, Cmd/Ctrl+A to select all, Delete to remove. Groups, minimap, toolbar, type-coloured edges, right-click spawn menu, and the imperative fitToContent / focusNode handle are all wired."
  height="660px"
...
<NodeGraph
  {...graph.bind}
  renderNode={node => (
    <NodeGraph.NodeBody accent={theme.accent}>
      <NodeGraph.NodeHeader title={node.data.title} subtitle={node.data.subtitle} />
      <NodeGraph.PinList>
...
>
  ...
  <NodeGraph.Minimap placement="bottom-right" width={220} nodeStyle={byCategory} />
  <NodeGraph.Toolbar placement="top-left">
    <NodeGraph.FitContentButton />
...
<PreviewToggle
  title="Minimal controlled graph"
  description="The bare bones: a nodes array and an edges array in plain useState, with inline NodeGraph.Port slots. Drag pin to pin to connect; drag a node and the wires follow; grab an edge end to reconnect."
  code={`const [nodes, setNodes] = useState([
  { id: 'value',  position: { x: -280, y: -30 }, data: {/* ... */} },
  { id: 'scale',  position: { x: -30,  y: -50 }, data: {/* ... */} },
  { id: 'output', position: { x: 230,  y: -30 }, data: {/* ... */} },
]);
const [edges, setEdges] = useState([
  { id: 'e1', source: { node: 'value', port: 'out' }, target: { node: 'scale', port: 'in' } },
  { id: 'e2', source: { node: 'scale', port: 'out' }, target: { node: 'output', port: 'in' } },
]);
  ...
  renderNode={node => (
    <div className="my-node">
      {node.data.ports.map(p =>
        p.side === 'left'
          ? <NodeGraph.Port key={p.id} id={p.id} side="left" />
          : <NodeGraph.Port key={p.id} id={p.id} side="right" />
      )}
    </div>
  )}
>
...
>
  <NodeGraphBasicsDemo client:only="react" />
</PreviewToggle>
...
<PreviewToggle
  title="Node anatomy — NodeBody / NodeHeader / PinList / Pin / NodeSection"
  description="A node built entirely from library compounds. NodeBody re-themes itself from the store (no ctx plumbing), PinList routes each Pin into its side column, and the collapsible NodeSection hides advanced pins — their edges hide with them and snap back on expand."
  code={`<NodeGraph.NodeBody accent="#5fd97e">
  <NodeGraph.NodeHeader title="Make Transform" subtitle="Pure" icon={<Fx />} />
  <NodeGraph.PinList columnGap={16}>
    <NodeGraph.Pin id="location" side="left"  dataType="vector" label="Location" />
    <NodeGraph.Pin id="rotation" side="left"  dataType="vector" label="Rotation" />
    <NodeGraph.Pin id="out"      side="right" dataType="object" label="Return Value" />
  </NodeGraph.PinList>

  <NodeGraph.NodeSection title="Advanced" defaultCollapsed>
    <NodeGraph.PinList columnGap={16}>
      <NodeGraph.Pin id="scale"   side="left" dataType="vector" label="Scale" />
      <NodeGraph.Pin id="uniform" side="left" dataType="float"  label="Uniform Scale" />
    </NodeGraph.PinList>
  </NodeGraph.NodeSection>
</NodeGraph.NodeBody>`}
>
  <NodeGraphAnatomyDemo client:only="react" />
</PreviewToggle>
...
<PreviewToggle
  title="Type-coloured wires, state emphasis, a dashed wire, and an animated pulse"
  description="edgeStyle colours each wire by its source pin's dataType, widens the selected/hovered wire, dashes the wire flagged async, and pulses the active exec wire's width over time (driven by a rAF + invalidate('edges')). renderEdgeLabel adds the chips. Switch the scheme up top."
  code={`const ref = useRef<NodeGraphHandle>(null);
...
<NodeGraph ref={ref} {...graph.bind} edgeStyle={edgeStyle} renderEdgeLabel={renderChip} />`}
>
  <NodeGraphEdgeStyleDemo client:only="react" />
</PreviewToggle>
...
<PreviewToggle
  title="Toolbar buttons + the imperative camera handle"
  description="The corner toolbar's Fit / zoom buttons call the same code path as the imperative handle. Below the graph, buttons call ref.fitToContent / focusNode / centerOn / zoomToRect directly; the live transform reads from ref.getTransform()."
  code={`const ref = useRef<NodeGraphHandle>(null);

<NodeGraph ref={ref} {...graph.bind} renderNode={renderNode}>
  <NodeGraph.Background variant="dots" />
  <NodeGraph.Toolbar placement="top-left">
    <NodeGraph.FitContentButton padding={64} />
    <NodeGraph.FitSelectionButton />
...
<Button onClick={() => ref.current?.focusNode('event')}>Focus: Event</Button>
<Button onClick={() => ref.current?.centerOn({ x: 0, y: 0 }, 1)}>Center origin</Button>
<Button onClick={() => ref.current?.zoomToRect({ x: 40, y: -260, width: 720, height: 380 })}>
  Frame region
</Button>`}
>
  <NodeGraphCameraDemo client:only="react" />
</PreviewToggle>
...
<PreviewToggle
  title="Right-click to spawn from a fuzzy-search palette"
  description="Register templates once; right-click empty space to open the palette (fuzzy filter + recent list + keyboard nav, all from the library). The picked node lands at the pointer's world point and is selected."
  code={`const TEMPLATES: NodeGraphTemplate[] = [
  {
    id: 'branch', title: 'Branch', group: 'Flow',
...
>
  <NodeGraphSpawnDemo client:only="react" />
</PreviewToggle>
...
<PreviewToggle
  title="Performance benchmark — scale the graph and watch the meter"
  description="Generates a grid of N nodes + edges. The FPS / frame-time / worst-frame meter measures the real browser cadence; the Orbit camera toggle pans the viewport every frame so every canvas layer redraws (the realistic per-frame cost). Bump the count and feel where it bends."
  height="640px"
  code={`const COUNTS = [120, 320, 640];
...
nodes.push({ id: 'n' + i, position: { x: col _ 210, y: row _ 120 }, width: 132, height: 58, data: {/* ... */} });
if (col > 0) edges.push({ id: 'e' + i, source: { node: 'n' + (i - 1), port: 'out' }, target: { node: 'n' + i, port: 'in' } });
}
return { nodes, edges };
}
...
<NodeGraph ref={ref} {...graph.bind} renderNode={renderNode} minZoom={0.05} responsive>
  <NodeGraph.Background variant="dots" gap={26} />
  <NodeGraph.Minimap placement="bottom-right" width={190} />
</NodeGraph>`}
>
  <NodeGraphBenchmarkDemo client:only="react" />
</PreviewToggle>
...
<PropsTable
  props={[
    ...
    },
    {
      ...
      description:
        'Fires with the full next edges array on connection drop, delete, reconnect.',
    },
    {
      name: 'groups',
      type: 'NodeGraphGroup[]',
    ...
    },
    {
      ...
      description:
        'Fires with the full next groups array (move, resize, rename, recolour).',
    },
    {
      name: 'selection',
    ...
    {
      name: 'edgeStyle',
      type: 'NodeGraphEdgeStyleFn',
      description:
        'Per-edge style hook run every draw frame. Return { color, width, dash }; ctx carries resolved source/target port side + dataType. Keep it cheap.',
    },
    {
    ...
    },
    {
      ...
      description:
        'Fires on pointer-up after a connection drag (success or cancel). Carries world + screen drop point.',
    },
    {
      name: 'onContextMenu',
    ...
    },
    {
      ...
      type: '(selection: NodeGraphSelection) => void',
      description:
        'Fires on Delete/Backspace when the selection is non-empty. Omit to let the library cascade-delete.',
    },
    {
      name: 'onActivate',
    ...
    },
    {
      ...
      description:
        'Slot subcomponents: Background, Minimap, Toolbar, SpawnPalette.',
    },
    {
      name: 'ref',
