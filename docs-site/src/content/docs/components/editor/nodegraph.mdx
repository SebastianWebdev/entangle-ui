...
<PreviewToggle
  ...
  renderNode={(node, ctx) => (
    <BlueprintNodeBody data={node.data} selected={ctx.selected}>
      {node.data.pins.map(pin => (
        <PinRow key={pin.id} side={pin.side}>
          {/* Slot is the single source of truth — the same DOM element the
              user clicks is where the edge anchors. */}
          <NodeGraph.Port id={pin.id} side={pin.side} dataType={pin.type}>
            <PinVisual type={pin.type} side={pin.side} />
          </NodeGraph.Port>
          <span>{pin.label}</span>
        </PinRow>
      ))}
