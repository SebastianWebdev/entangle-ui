...
<PreviewToggle
  ...
  renderEdgeLabel={edge => <Chip>{edge.data.label}</Chip>}
>
...
<PreviewToggle
  ...
  description="Two stress axes: Grid scales the node count (one edge per node); Layered net keeps nodes low but fully-connects each layer for a flood of edges (2·20·50·20·2 = 94 nodes, 2080 edges). The Orbit camera toggle redraws every canvas layer each frame; the meter reports the real browser cadence."
  height="640px"
  code={`<NodeGraph
  ref={ref}
  {...graph.bind}
