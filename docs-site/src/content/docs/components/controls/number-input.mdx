...
<PropsTable props={[
  ...
  { name: 'unit', type: "string | ((value: number) => string)", description: 'Unit suffix to display (e.g., "px", "%", "deg"). Can be a function for dynamic formatting.' },
  { name: 'showStepButtons', type: 'boolean', default: 'true', description: 'Whether to show increment/decrement chevron buttons on hover.' },
  { name: 'validate', type: '(value: number) => string | undefined', description: 'Custom validation function. Return error message string if invalid.' },
  { name: 'formatValue', type: '(value: number) => string', description: 'Format value for display (e.g., add commas, currency symbols).' },
  { name: 'parseValue', type: '(input: string) => number | null', description: 'Parse custom formatted input back to number. Used with formatValue.' },
  { name: 'min', type: 'number', description: 'Minimum allowed value (hard limit).' },
  { name: 'max', type: 'number', description: 'Maximum allowed value (hard limit).' },
  { name: 'softMin', type: 'number', description: 'Soft minimum for drag operations. Can be overridden by keyboard input.' },
  { name: 'softMax', type: 'number', description: 'Soft maximum for drag operations. Can be overridden by keyboard input.' },
  { name: 'step', type: 'number', default: '1', description: 'Step size for increment/decrement and drag operations.' },
  { name: 'precisionStep', type: 'number', default: 'step / 10', description: 'Step size when Shift is held (precision mode).' },
  { name: 'largeStep', type: 'number', default: 'step * 10', description: 'Step size when Ctrl is held (large steps).' },
  { name: 'precision', type: 'number', description: 'Number of decimal places for display.' },
  { name: 'allowExpressions', type: 'boolean', default: 'true', description: 'Whether to allow mathematical expression input (pi, sqrt, sin, etc.).' },
  { name: 'dragSensitivity', type: 'number', default: '1', description: 'Sensitivity multiplier for mouse drag scrubbing.' },
  ...
  { name: 'placeholder', type: 'string', description: 'Placeholder text shown in edit mode.' },
  ...
  { name: 'onKeyDown', type: '(event: KeyboardEvent) => void', description: 'Key down event handler.' },
  ...
  { name: 'ref', type: 'Ref<HTMLInputElement>', description: 'Ref to the underlying input element.' },
]} />
