...
<Combobox
  creatable
  createLabel={q => <>Add new tag: <strong>{q}</strong></>}
  options={options}
...
```tsx
<Combobox name="country" options={countries} value={country} onChange={setCountry} />
```
...
<PropsTable
  props={[
    ...
    },
    {
      ...
      description: 'Render the create row label. Defaults to `Create "<query>"`.',
    },
    {
      name: 'openOnFocus',
      type: 'boolean',
