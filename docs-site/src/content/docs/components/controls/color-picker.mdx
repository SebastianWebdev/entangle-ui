...
<PropsTable props={[
  { name: 'value', type: 'string', description: 'Current color value (controlled). Accepts any valid CSS color string.' },
  { name: 'defaultValue', type: 'string', default: "'#007acc'", description: 'Default color value (uncontrolled).' },
  { name: 'format', type: "'hex' | 'rgb' | 'hsl'", default: "'hex'", description: 'Output format for the onChange callback.' },
  { name: 'showAlpha', type: 'boolean', default: 'false', description: 'Whether to show the alpha channel slider.' },
  { name: 'inputModes', type: "('hex' | 'rgb' | 'hsl')[]", default: "['hex', 'rgb', 'hsl']", description: 'Available input mode tabs in the picker.' },
  { name: 'defaultInputMode', type: "'hex' | 'rgb' | 'hsl'", default: "'hex'", description: 'Default active input mode.' },
  { name: 'presets', type: 'ColorPreset[]', description: 'Array of preset colors for quick selection. Each preset has color and optional label.' },
  { name: 'palette', type: "string | Palette | PaletteColor[]", description: 'Built-in palette name, custom Palette object, or array of PaletteColor.' },
  { name: 'showEyeDropper', type: 'boolean', default: 'false', description: 'Whether to show the EyeDropper button (Chromium-only).' },
  { name: 'size', type: "'sm' | 'md' | 'lg'", default: "'md'", description: 'Size of the swatch trigger.' },
  { name: 'swatchShape', type: "'square' | 'circle'", default: "'square'", description: 'Shape of the swatch trigger.' },
  { name: 'label', type: 'string', description: 'Label displayed next to the swatch trigger.' },
  { name: 'disabled', type: 'boolean', default: 'false', description: 'Whether the picker is disabled.' },
  { name: 'inline', type: 'boolean', default: 'false', description: 'Whether to render the picker inline (no popover).' },
  { name: 'pickerWidth', type: 'number', default: '240', description: 'Width of the picker panel in pixels.' },
  { name: 'onChange', type: '(color: string) => void', description: 'Callback fired continuously as the color changes.' },
  { name: 'onChangeComplete', type: '(color: string) => void', description: 'Callback fired when a color change is committed (drag end).' },
