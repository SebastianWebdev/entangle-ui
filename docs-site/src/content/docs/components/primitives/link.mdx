...
`disabled` renders as a `<span>` with no `href`, sets `aria-disabled`, and turns off pointer events. Browsers will not navigate, and assistive tech will announce the disabled state.
...
<PropsTable
  props={[
    ...
    },
    {
      name: 'as',
      type: 'ElementType',
      default: "'a' (or 'span' when disabled)",
      description:
        "Polymorphic root override — pass a router's link component to get client-side navigation.",
    },
    {
      name: 'variant',
