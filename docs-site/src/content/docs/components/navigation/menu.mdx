...
<PropsTable
  props={[
    ...
    },
    {
      ...
      default: "'start'",
      description:
        'Alignment along the chosen side. Defaults to start, so the menu edge lines up with the trigger edge instead of centering.',
    },
    {
      name: 'sideOffset',
      type: 'number',
      default: 'Menu gap (8)',
      description:
        'Gap in px between this popup and its anchor. Defaults to the Menu `gap`; set this to override a single popup.',
    },
    {
      name: 'alignOffset',
      type: 'number',
