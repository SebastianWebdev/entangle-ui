...
<PropsTable props={[
  { name: 'config', type: 'MenuConfig | (context: ContextMenuTargetDetails<TPayload>) => MenuConfig', description: 'Static menu config or resolver function called with trigger context.' },
  { name: 'selectedItems', type: 'Record<string, string[]>', description: 'Currently selected items grouped by group ID.' },
  { name: 'onChange', type: '(selection: MenuSelection) => void', description: 'Callback when selection state changes.' },
  { name: 'children', type: 'ReactNode', description: 'Content that acts as the right-click trigger area.' },
  { name: 'payload', type: 'TPayload', description: 'Optional data attached to this trigger area, passed to the config resolver.' },
  ...
  { name: 'disabled', type: 'boolean', default: 'false', description: 'Disables opening the context menu.' },
  { name: 'className', type: 'string', description: 'Additional CSS class names for the menu popup.' },
  { name: 'style', type: 'CSSProperties', description: 'Inline styles for the menu popup.' },
  { name: 'testId', type: 'string', description: 'Test identifier for automated testing.' },
  { name: 'ref', type: 'Ref<HTMLDivElement>', description: 'Ref to the menu popup element.' },
]} />
