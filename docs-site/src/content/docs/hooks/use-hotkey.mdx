...
<PropsTable
  props={[
    ...
    },
    {
      ...
      description:
        'Override the listener target. Defaults to `window`. Use a specific element to scope the hotkey.',
    },
  ]}
/>
...
- **`target` lifecycle:** When passing a custom `target`, make sure the element is mounted by the time the effect runs. Pass the ref's `.current` value or render the consumer below the element.
