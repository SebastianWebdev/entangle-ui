...
Import the bundled stylesheet once at your app entry to register every `--etui-*` token on `:root`. No provider is needed for the default look:

```tsx
import 'entangle-ui/styles.css';
import { Button } from 'entangle-ui';

// Works out of the box — the default dark theme is active
