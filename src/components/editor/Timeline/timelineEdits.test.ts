...
describe('timelineEdits', () => {
  ...
  describe('setKeyframeTangent', () => {
    ...
    it('clamps the dragged handle so the control point stays in valueRange', () => {
      // valueRange [0, 1], kf.y = 0.5 → handle.y must be in [-0.5, 0.5].
      // Drag pulls way up (offset.y = +5) → clamped to 0.5.
      const tracks: TimelineTrack[] = [
        {
          id: 't1',
          valueRange: [0, 1],
      ...
      const a = track(next, 't1').keyframes.find(k => k.id === 'a');
      expect(a?.handleOut.y).toBe(0.5);
      expect(a?.handleOut.x).toBe(4);
    });

    it('clamps both handles when mirrored mode would push the opposite out of range', () => {
      // valueRange [0, 1], kf.y = 0.5, mirrored → opposite = -offset. Drag
      // offset.y = +0.8 would force opposite.y = -0.8 (escaping below 0).
      // First we clamp offset itself (0.5 max), then mirroring writes -0.5,
      // which still violates the lower bound 0; the post-clamp catches it.
      const tracks: TimelineTrack[] = [
        {
          id: 't1',
          valueRange: [0, 1],
      ...
      const a = track(next, 't1').keyframes.find(k => k.id === 'a');
      // Both handles' y-offsets bounded by [-0.5, 0.5] so neither control
      // point escapes the lane.
      expect(a?.handleOut.y).toBeLessThanOrEqual(0.5);
      expect(a?.handleOut.y).toBeGreaterThanOrEqual(-0.5);
      expect(a?.handleIn.y).toBeLessThanOrEqual(0.5);
      expect(a?.handleIn.y).toBeGreaterThanOrEqual(-0.5);
    });

    it('preserves identity when the clamp would be a no-op', () => {
      const tracks: TimelineTrack[] = [
        { id: 't1', valueRange: [0, 1], keyframes: [kf('a', 10, 0.5)] },
      ];
      // Inside the bounds — handleOut.y = 0.2 is fine, no change beyond
      // the tangent itself.
      const next = setKeyframeTangent(
        tracks,
        { trackId: 't1', keyframeId: 'a' },
