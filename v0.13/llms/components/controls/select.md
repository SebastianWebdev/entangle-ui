# Select
> Dropdown select component with searchable mode, grouped options, keyboard navigation, and clearable state.
...
| `minDropdownWidth` | `number` | — | Minimum width of the dropdown in pixels. The dropdown still grows past this to fit its content. |
| `fullWidth` | `boolean` | `false` | Whether the select stretches to fill the width of its container. |
| `name` | `string` | — | Name attribute for hidden input (form submission). |
| `onChange` | `(value: T) => void` | — | Callback when the selected value changes. The value type is discriminated by `clearable`: a non-clearable select emits `T`; a `clearable` select emits `T | null` (null comes from the clear button). |
