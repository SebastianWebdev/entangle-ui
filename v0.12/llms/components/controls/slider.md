...
| `showTooltip` | `boolean` | `true` | Show value tooltip while dragging. The tooltip is portaled to document.body, so it is never clipped by an ancestor with overflow (e.g. a panel/section header). |
| `tooltipPlacement` | `'top' \| 'bottom'` | `'top'` | Placement of the value tooltip relative to the thumb. |
