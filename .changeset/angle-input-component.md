...
- `GradientEditor`'s angle control (linear/conic) is now an `AngleInput` dial with a read-only value readout — no editable number field — replacing the old narrow input; `onChangeComplete` also fires on angle drag-end.
