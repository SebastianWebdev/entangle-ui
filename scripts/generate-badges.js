...
function generateBadges() {
  try {
    const badgesData = JSON.parse(fs.readFileSync('test_results/badges.json', 'utf8'));
    ...
    const badgeMarkdown = shieldsUrls.map(badge => 
      `![${badge.label}](${badge.url})`
    ).join(' ');
    
    // Generate badge HTML (for more control)
    const badgeHtml = shieldsUrls.map(badge => 
    ...
    fs.writeFileSync('test_results/badges-output.json', JSON.stringify(output, null, 2));
    ...
    console.log('💡 Run npm run test:package-report first to generate badge data');
  }
}
