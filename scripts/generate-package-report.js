...
function generatePackageReport() {
  try {
    ...
    try {
      testResults = JSON.parse(fs.readFileSync('test_results/coverage-results.json', 'utf8'));
    } catch (e) {
      console.warn('⚠️  No test results found. Run npm run test:coverage first.');
    }

    try {
      coverageSummary = JSON.parse(fs.readFileSync('test_results/coverage/coverage-summary.json', 'utf8'));
    } catch (e) {
    ...
    const report = {
      ...
      quality: {
        ...
        hasCoverage: coverageSummary ? coverageSummary.total?.lines?.pct > 0 : false,
        hasTypeScript: fs.existsSync('tsconfig.json'),
        hasLinting: fs.existsSync('eslint.config.js') || fs.existsSync('.eslintrc.js'),
        hasPrettier: fs.existsSync('prettier.config.js') || fs.existsSync('.prettierrc'),
        hasStorybook: fs.existsSync('.storybook'),
        hasContinuousIntegration: fs.existsSync('.github/workflows') || fs.existsSync('.gitlab-ci.yml')
      },
      files: {
    ...
    // Save full report
    fs.writeFileSync('test_results/package-report.json', JSON.stringify(report, null, 2));
    
    // Save badges data
    fs.writeFileSync('test_results/badges.json', JSON.stringify(badges, null, 2));
    ...
    console.log('   • test_results/README-quality-section.md - README quality section');
...
function generateBadges(report) {
  ...
  if (report.coverage) {
    ...
    badges.coverage = {
      ...
      color: linesCoverage >= 90 ? 'brightgreen' : 
             linesCoverage >= 80 ? 'yellow' : 
             linesCoverage >= 70 ? 'orange' : 'red'
    };
  }
...
function printReportSummary(report) {
  ...
  if (report.testing) {
    ...
    console.log(`   Tests: ${report.testing.passedTests}/${report.testing.totalTests} passing (${report.testing.passRate})`);
    console.log(`   Suites: ${report.testing.passedSuites}/${report.testing.totalSuites} passing`);
  }
  ...
  Object.entries(report.quality).forEach(([key, value]) => {
    const emoji = value ? '✅' : '❌';
    console.log(`   ${emoji} ${key.replace(/([A-Z])/g, ' $1').replace(/^./, str => str.toUpperCase())}`);
  });
}
