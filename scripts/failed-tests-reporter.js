...
export default class FailedTestsReporter {
  onFinished(files = [], errors = []) {
          ...
          tasks.forEach(task => {
            if (task.type === 'test' && task.result?.state === 'fail') {
              failedSuite.assertionResults.push({
                ...
                failureMessages: task.result.errors ? task.result.errors.map(e => e.message || e.toString()) : [],
                meta: {}
              });
            }
    ...
    const summary = {
      ...
      numFailedTests: failedTests.reduce((acc, suite) => acc + suite.assertionResults.length, 0),
      startTime: Date.now(),
    ...
    if (failedTests.length > 0) {
      ...
      console.log(`❌ ${summary.numFailedTests} failed tests in ${summary.numFailedTestSuites} suites`);
    } else {
